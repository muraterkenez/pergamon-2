...
export function EditAnimalModal({ isOpen, onClose, onSuccess, animal }: EditAnimalModalProps) {
  const { user } = useAuth();
  const { register, handleSubmit, watch, setValue, formState: { errors, isSubmitting }, reset } = useForm<EditAnimalFormData>({
    defaultValues: {
      ...
      source: animal.source as 'birth' | 'purchase',
      ...
      health_status: animal.health_status as 'healthy' | 'sick' | 'treatment',
      notes: animal.notes || '',
      status: animal.status as 'active' | 'sold' | 'deceased',
      pedigree_registered: animal.pedigree_registered || false,
      pedigree_number: animal.pedigree_number || '',
      genetic_merit: animal.genetic_merit || 0,
      milk_yield_potential: animal.milk_yield_potential || 0,
      fat_percentage: animal.fat_percentage || 0,
      protein_percentage: animal.protein_percentage || 0,
      body_condition_score: animal.body_condition_score || 0,
      is_insemination: animal.is_insemination || false
    }
  });
  ...
  const onSubmit = async (data: EditAnimalFormData) => {
    try {
      setError(null);
      ...
      setError('Hayvan güncellenirken bir hata oluştu. Lütfen tekrar deneyin.');
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-blue-50 border-l-4 border-blue-400 p-4">
        <div className="flex">
          ...
          <div className="ml-3">
            <p className="text-sm text-blue-700">
              Hayvan kaydı güncellendikten sonra fotoğraf ekleyebilir ve sağlık kayıtlarını düzenleyebilirsiniz.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg w-full max-w-2xl">
        <div className="border-b p-4 sm:p-6">
          <div className="flex justify-between items-center">
            <h2 className="text-xl font-semibold text-gray-900">Hayvan Düzenle</h2>
            <button 
              onClick={onClose}
